...
EXPORT DOINLINE CONST vfloat xfabsf(vfloat x) { return vabs_vf_vf(x); }

EXPORT DOINLINE CONST vfloat xcopysignf(vfloat x, vfloat y) { return vcopysign_vf_vf_vf(x, y); }

EXPORT DOINLINE CONST vfloat xfmaxf(vfloat x, vfloat y) {
  return vsel_vf_vo_vf_vf(visnan_vo_vf(y), x, vsel_vf_vo_vf_vf(vgt_vo_vf_vf(x, y), x, y));
}

EXPORT DOINLINE CONST vfloat xfminf(vfloat x, vfloat y) {
  return vsel_vf_vo_vf_vf(visnan_vo_vf(y), x, vsel_vf_vo_vf_vf(vgt_vo_vf_vf(y, x), x, y));
}

EXPORT DOINLINE CONST vfloat xfdimf(vfloat x, vfloat y) {
...
EXPORT DOINLINE CONST vfloat xtruncf(vfloat x) {
  vfloat fr = vsub_vf_vf_vf(x, vcast_vf_vi2(vtruncate_vi2_vf(x)));
...
EXPORT DOINLINE CONST vfloat xrintf(vfloat d) {
  vfloat x = vadd_vf_vf_vf(d, vcast_vf_f(0.5f));
...
EXPORT DOINLINE CONST vfloat xfmaf(vfloat x, vfloat y, vfloat z) {
...
EXPORT DOINLINE CONST vfloat xsqrtf_u05(vfloat d) {
#if defined(ENABLE_AVX) || defined(ENABLE_AVX2) || defined(ENABLE_AVX512) || defined(ENABLE_ADVSIMD) || defined(ENABLE_NEON)
...
EXPORT DOINLINE CONST vfloat xhypotf_u05(vfloat x, vfloat y) {
  x = vabs_vf_vf(x);
  y = vabs_vf_vf(y);
...
EXPORT DOINLINE CONST vfloat xnextafterf(vfloat x, vfloat y) {
...
EXPORT DOINLINE CONST vfloat xfrfrexpf(vfloat x) {
...
EXPORT DOINLINE CONST vint2 xexpfrexpf(vfloat x) {
  /*
...
EXPORT DOINLINE CONST vfloat xfmodf(vfloat x, vfloat y) {
