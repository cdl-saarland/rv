...
VectorShape
operator+ (const VectorShape& a, const VectorShape& b)
{
...
operator- (const VectorShape& a, const VectorShape& b)
{
	const int aligned = VectorShape::gcd(a.alignment, b.alignment);
...
VectorShape
operator* (int strideMultiplier, const VectorShape& a)
{
...
VectorShape
operator/ (const VectorShape& a, int strideDivisor)
{
	const int aligned = (a.alignment % strideDivisor == 0) ? (a.alignment / strideDivisor) : 1;
...
bool
VectorShape::operator==(const VectorShape& a) const
{
	return hasConstantStride == a.hasConstantStride &&
		   stride == a.stride &&
