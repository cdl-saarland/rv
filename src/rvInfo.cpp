...
bool
RVInfo::addCommonMappings(const bool useSSE,
                           const bool useSSE41,
                           const bool useSSE42,
                           const bool useAVX,
                           const bool useNEON)
{
	// FIXME work-around
#if 0
	if (mModule->getFunction("log2_ps") || mModule->getFunction("exp256_ps")) {
		mRVLibLinked = true;
	}
#endif
    ...
    if (!mRVLibLinked)
    {
        // Load module from path in build-system-defined variable.
        Module * rvLibModule = rv::createModuleFromFile(RV_RVLIB_FILE, *mContext);
        // Link lib module into main module.

        if (rvLibModule){
        	// FIXME this is ugly
        	// Check whether we will get linker errors attempting to link in the rvRuntimeLibrary
        	bool abortLinking = false;
        	for (Function & func : *mModule) {
        		std::string funcName = func.getName();
        		if (Function * conflictFunc = rvLibModule->getFunction(funcName)) {

        			// function already defined
        			// We preempt here to avoid linker crashes later on
        			if (! func.isDeclaration() ) {
        				errs() << "WARNING: Symbol " << funcName << " already defined. Will erase body before linking RV runtime library.\n";
        				func.deleteBody();
        			}

					if (conflictFunc->getFunctionType() != func.getFunctionType()) {
						errs() << "ERROR: Mismatch in conflicting symbol " << funcName
								<< ":\n runtimeDecl: " << conflictFunc->getFunctionType()
								<< "\n existing Decl: " << func.getFunctionType() << "\n";
						abort();
						return false;
					}
        		}
            ...
            Linker linker(*mModule);
				//[](const DiagnosticInfo &DI) { assert(false && "an error occured while linking"); }
			std::string error = "";
			const bool failure = linker.linkInModule(std::unique_ptr<Module>(rvLibModule));
			if (failure) {
				errs() << "ERROR: linking of RV library failed: " << error << "\n";
				return false;
			}
        }
        else {
        	mRVLibLinked = false;
            errs() << "ERROR: Could not find RV runtime library at path " << RV_RVLIB_FILE << " !\n";
            abort();
            return false;
        }
