...
AdjustStride(Loop & loop, PHINode & phi, uint vectorWidth) {
  ...
  for (uint i = 0; i < phi.getNumIncomingValues(); ++i) {
    ...
    if (!inInst || (inInst->getOpcode() != Instruction::Add)) continue;
    increment = inInst;
    break;
  }
...
void
vectorizeLoop(Function & parentFn, Loop & loop, uint vectorWidth, LoopInfo & loopInfo, DFG & dfg, CDG & cdg, DominatorTree & domTree, PostDominatorTree & postDomTree) {
...
// set-up for loop vectorization
  ...
  vecInfo.setVectorShape(*cast<BranchInst>(exitBlock->getTerminator())->getOperand(0), rv::VectorShape::uni());
...
// control conversion
  bool linearizeOk = vectorizer.linearizeCFG(vecInfo, *maskAnalysis, loopInfo, postDomTree, domTree);
  assert(linearizeOk);

  const DominatorTree domTreeNew(*vecInfo.getMapping().scalarFn); // Control conversion does not preserve the domTree so we have to rebuild it for now
...
vectorizeType(Type * scalarTy, rv::VectorShape shape, uint vectorWidth) {
  ...
  if (!shape.isDefined() || shape.hasStridedShape()) {
    return scalarTy;
  }
...
Function *
createVectorDeclaration(Function & scalarFn, rv::VectorShape resShape, const rv::VectorShapeVec & argShapes, uint vectorWidth) {
  ...
  return llvm::Function::Create(vectorFnTy, scalarFn.getLinkage(), scalarFn.getName() + "_SIMD", scalarFn.getParent());
}

rv::VectorShape
decodeShape(char c) {
  if (c == 'C') return rv::VectorShape::cont();
  else if (c == 'T') return rv::VectorShape::varying();
  else if (c == 'U') return rv::VectorShape::uni();
...
int main(int argc, char ** argv)
{
	...
	if (! (hasFile && hasKernelName)) {
		std::cerr << "Not all arguments specified -wfv/-loopvec) -i MODULE -k KERNELNAME [-target TARGET_DECL] [-o OUTPUT_LL] [-w 8] [--vectorize] [--analyze\n";
		return -1;
	}
...
// initialize argument mapping
        ...
        if (reader.readOption<std::string>("-s", shapeText)) {
          ...
          if (shapeText.size() == i + 2) { // return shape
            assert(shapeText[i] == 'r' && "expected return shape");
            resShape = decodeShape(shapeText[i+1]);
          }
...
// Create simd decl
	...
	if (! hasTargetDeclName) {
		errs() << "No target function declaration given (-target <NAME>), creating declaration with same signature " << scalarFn->getName() << "_SIMD(..) \n";
                vectorFn = createVectorDeclaration(*scalarFn, resShape, argShapes, vectorWidth);
	} else {
		vectorFn = mod->getFunction(targetDeclName);
                // TODO verify shapes
		if (!vectorFn) {
			llvm::errs() << "Target declaration " << targetDeclName << " not found. Aborting!\n";
			return 3;
		}
	}
        assert(vectorFn);
...
// Vectorize
	errs() << "\nVectorizing kernel \"" << vectorizerJob.scalarFn->getName() << "\" into declaration \"" << vectorizerJob.vectorFn->getName() <<  "\" with vector size " << vectorizerJob.vectorWidth << "... \n";
	...
	//output
	if (hasOutFile) { writeModuleToFile(mod, outFile); errs() << "Final module written to \"" << outFile << "\"\n"; }
	else mod->dump();
